<x-backend.layouts.master>
    ...
    <x-slot name='breadCrumb'>
        <x-backend.layouts.elements.breadcrumb>
            <x-slot name="pageHeader"> Production Data Graph</x-slot>
    ...
    <script>
        // First Doughnut Chart: Most Alter on Line
        const ctx_one = document.getElementById('alterationChart').getContext('2d');
        const alterationData = @json($alterationData);

        const alterationLabels = [];
        const alterationDataPoints = [];

        alterationData.forEach(item => {
            const totalAlters = Object.values(item).slice(1).reduce((acc, val) => acc + (val || 0), 0);
            alterationLabels.push(item.line);
            alterationDataPoints.push(totalAlters);
        });
        ...
        // Second Doughnut Chart: Most Reject on Line
        const ctx_two = document.getElementById('RJChart').getContext('2d');
        const rejectionData = @json($rejectionData);

        const rejectionLabels = [];
        const rejectionDataPoints = [];

        rejectionData.forEach(item => {
            const totalRejects = Object.values(item).slice(1).reduce((acc, val) => acc + (val || 0), 0);
            rejectionLabels.push(item.line);
            rejectionDataPoints.push(totalRejects);
        });
        ...
        // Bar Chart: Total per Line
        const ctx = document.getElementById('rejectionBarChart').getContext('2d');
        ...
        totalData.forEach(item => {
            totalLabels.push(item.line);
            totalDataPoints.push(item.total_rejections);
        });
