<x-backend.layouts.master>
    ...
    <x-slot name='breadCrumb'>
        <x-backend.layouts.elements.breadcrumb>
            ...
            <li class="breadcrumb-item"><a href="{{ route('planning_data.index') }}">Edit & Update Planning Data</a></li>
            <li class="breadcrumb-item active">Edit & Update Planning Data</li>
        </x-backend.layouts.elements.breadcrumb>
    </x-slot>


    <x-backend.layouts.elements.errors />
    <form action="{{ route('planning_data_update', $planning_data->id) }}" method="post" enctype="multipart/form-data">
        @csrf
        @method('PUT') <!-- Update Method -->
            ...
            <div class="container-fluid">
                ...
                <table class="table table-bordered table-hover" id="dynamicTable">
                    ...
                    <tbody>
                        <tr>
                            <input type="hidden" name="planning_data_id" value="{{ $planning_data->id }}">
                            <td>
                                ...
                                <select name="floor[]" id="floor" class="form-control" required>
                                    ...
                                    @foreach (range(1, 5) as $floor)
                                        <option value="{{ $floor }}"
                                            {{ $planning_data->floor ?? old('floor') == $floor ? 'selected' : '' }}>
                            ...
                            <td>
                                <input type="text" name="line[]" class="form-control" required
                                    value="{{ $planning_data->line ?? old('line') }}">

                            </td>
                            <td>
                                <select name="buyer_id[]" class="form-control buyer-select" required>
                                    <option value="">Select Buyer</option>
                                    <option value="new_buyer" {{ $planning_data->buyer_id ?? old('buyer_id') == 'new_buyer' ? 'selected' : '' }}>
                                        Other</option>
                                    @foreach ($buyers as $buyer)
                                        <option value="{{ $buyer->id }}"
                                            {{ $planning_data->buyer_id ?? old('buyer_id') == $buyer->id ? 'selected' : '' }}>
                                            {{ $buyer->name }}
                                        </option>
                                    @endforeach
                                </select>
                                <input type="text" name="new_buyer_name[]" class="form-control buyer-input"
                                    ...
                                    value="{{ $planning_data->buyer ?? old('new_buyer_name') }}">
                            </td>
                            <!-- Style Column -->
                            <td>
                                <select name="style[]" class="form-control style-select" required>
                                    <option value="">Select Order / Style</option>
                                    <option value="new_style" {{ $planning_data->style ?? old('style') == 'new_style' ? 'selected' : '' }}>
                                        Other</option>
                                    @foreach ($orders as $order)
                                        <option value="{{ $order->style }}"
                                            {{ $planning_data->style ?? old('style') == $order->style ? 'selected' : '' }}>
                                            {{ $order->style }}
                                        </option>
                                    @endforeach
                                </select>
                                <input type="text" name="new_style_name[]" class="form-control style-input"
                                    ...
                                    value="{{ $planning_data->style ?? old('new_style_name') }}">
                            </td>
                            <!-- Item Column -->
                            <td>
                                <select class="form-control item-select" name="item[]" required>
                                    <option value="">Select Item</option>
                                    <option value="other" {{ $planning_data->item ?? old('item') == 'other' ? 'selected' : '' }}>Other
                                    </option>
                                    <option value="hat" {{ $planning_data->item ?? old('item') == 'hat' ? 'selected' : '' }}>Hat
                                    </option>
                                    <option value="tanktop" {{ $planning_data->item ?? old('item') == 'tanktop' ? 'selected' : '' }}>
                                        Tanktop</option>
                                    <option value="racer" {{ $planning_data->item ?? old('item') == 'racer' ? 'selected' : '' }}>Racer
                                    </option>
                                    <option value="boxer" {{ $planning_data->item ?? old('item') == 'boxer' ? 'selected' : '' }}>Boxer
                                    </option>
                                    <option value="leggings" {{ $planning_data->item ?? old('item') == 'leggings' ? 'selected' : '' }}>
                                        Leggings</option>
                                    <option value="trouser" {{ $planning_data->item ?? old('item') == 'trouser' ? 'selected' : '' }}>
                                        Trouser</option>
                                    <option value="joggers" {{ $planning_data->item ?? old('item') == 'joggers' ? 'selected' : '' }}>
                                        Joggers</option>
                                    <option value="t-shirt" {{ $planning_data->item ?? old('item') == 't-shirt' ? 'selected' : '' }}>T
                                        Shirt</option>
                                    <option value="reversible-t-shirt"
                                        {{ $planning_data->item ?? old('item') == 'reversible-t-shirt' ? 'selected' : '' }}>Reversible T
                                        Shirt</option>
                                    <option value="reversible-pant"
                                        {{ $planning_data->item ?? old('item') == 'reversible-pant' ? 'selected' : '' }}>Reversible Pant
                                    </option>
                                    <option value="romper" {{ $planning_data->item ?? old('item') == 'romper' ? 'selected' : '' }}>Romper
                                    </option>
                                    <option value="sweat-shirt" {{ $planning_data->item ?? old('item') == 'sweat-shirt' ? 'selected' : '' }}>
                                        Sweat Shirt</option>
                                    <option value="polo-shirt" {{ $planning_data->item ?? old('item') == 'polo-shirt' ? 'selected' : '' }}>Polo
                                        Shirt</option>
                                    <option value="jacket" {{ $planning_data->item ?? old('item') == 'jacket' ? 'selected' : '' }}>Jacket
                                    </option>
                                    <option value="hoody-jacket" {{ $planning_data->item ?? old('item') == 'hoody-jacket' ? 'selected' : '' }}>
                                        Hoody Jacket</option>
                                    <option value="frocks" {{ $planning_data->item ?? old('item') == 'frocks' ? 'selected' : '' }}>Frocks
                                    </option>
                                    <option value="cargo-pant" {{ $planning_data->item ?? old('item') == 'cargo-pant' ? 'selected' : '' }}>
                                        Cargo Pant</option>
                                    <option value="blazer" {{ $planning_data->item ?? old('item') == 'blazer' ? 'selected' : '' }}>Blazer
                                    </option>
                                    <!-- Other options for Item array from database -->
                                    @foreach ($items as $item)
                                        <option value="{{ $item->item }}"
                                            {{ $planning_data->item ?? old('item') == $item->item ? 'selected' : '' }}>
                                            {{ $item->item }}
                                        </option>
                                    @endforeach
                            ...
                            <!-- Order Qty Column -->
                            <td>
                                <input type="number" name="smv[]" class="form-control" step="0.01" required
                                    value="{{ $planning_data->smv ?? old('smv') }}">
                            </td>
                            <td>
                                <input type="number" name="order_qty[]" class="form-control" required
                                    value="{{ $planning_data->order_qty ?? old('order_qty') }}">
                            </td>
                            <!-- Allocate Qty Column -->
                            <td>
                                <input type="number" name="allocate_qty[]" class="form-control" required
                                    value="{{ $planning_data->allocate_qty ?? old('allocate_qty') }}">
                            <!-- Sewing Start Column -->
                            <td>
                                <input type="date" name="sewing_start[]" class="form-control" required
                                    value="{{ $planning_data->sewing_start_time ?? old('sewing_start') }}">
                            </td>
                            <!-- Sewing End Column -->
                            <td>
                                <input type="date" name="sewing_end[]" class="form-control" required
                                    value="{{ $planning_data->sewing_end_time ?? old('sewing_end') }}">
                            </td>
                            <!-- Required Man Power Column -->
                            <td>
                                <input type="number" name="required_man_power[]" class="form-control" 
                                    value="{{ $planning_data->required_man_power ?? old('reuired_man_power') }}">
                            </td>
                            <!-- Average Working Hour Column -->
                            <td>
                                <input type="number" name="average_working_hour[]" class="form-control" 
                                    value="{{ $planning_data->average_working_our ?? old('average_working_hour') }}">
                            </td>
                            <!-- Expected Efficiency Column -->
                            <td>
                                <input type="number" name="expected_efficiency[]" class="form-control" 
                                    value="{{ $planning_data->expected_efficiency ?? old('expected_efficiency') }}">
                            </td>
                            <!-- Remarks Column -->
                            <td>
                                <input type="text" name="remarks[]" class="form-control" 
                                    value="{{ $planning_data->remarks ?? old('remarks') }}">
                            </td>
